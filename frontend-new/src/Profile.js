...
function Profile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const userResponse = await axios.get("https://code-summarizer.onrender.com/user_info", {
          headers: { Authorization: `Bearer ${token}` },
        });

        setUserInfo(userResponse.data);

        const summariesResponse = await axios.get("https://code-summarizer.onrender.com/view_saved_sum", {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDelete = async (sumid) => {
    ...
    try {
      await axios.delete(`https://code-summarizer.onrender.com/delete_summary/${sumid}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
